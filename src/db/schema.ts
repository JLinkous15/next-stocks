import { pgTable, varchar, boolean, uuid, timestamp, text, primaryKey } from "drizzle-orm/pg-core";
import { relations, sql } from 'drizzle-orm'

export const users = pgTable("users", {
    id: uuid("user_id").primaryKey().default(sql`gen_random_uuid()`),
...
export const usersToTickers = pgTable("user_to_tickers", {
    userId: uuid('user_id')
    .notNull()
    .references(() => users.id),
